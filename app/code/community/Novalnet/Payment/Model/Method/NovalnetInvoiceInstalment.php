 ...
 */
class Novalnet_Payment_Model_Method_NovalnetInvoiceInstalment extends Novalnet_Payment_Model_Method_Abstract
{
    protected $_code = Novalnet_Payment_Model_Config::NN_INVOICE_INSTALMENT;
    protected $_canAuthorize = Novalnet_Payment_Model_Config::NN_INVOICE_INSTALMENT_CAN_AUTHORIZE;
    protected $_formBlockType = Novalnet_Payment_Model_Config::NN_INVOICE_INSTALMENT_FORM_BLOCK;
    protected $_infoBlockType = Novalnet_Payment_Model_Config::NN_INVOICE_INSTALMENT_INFO_BLOCK;
    ...
    public function assignData($data)
    {
        if ($data) {
            ...
            if ($data->getDobDate() && $data->getDobMonth() && $data->getDobYear()) {
                ...
                $methodSession->setInvoiceCustomerDob($birthDate);
            }

            $instalmentPeriod = $this->getFormValues('novalnet_invoice_instalment_period');
            $methodSession->setInvoiceInstalment($instalmentPeriod);
